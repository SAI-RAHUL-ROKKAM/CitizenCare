...
const DEPT_ICONS = {
  Hospital: '🏨', Police: '🚔', Sanitation: '🗑️', Roads: '🛣️',
  Water: '💧', Electricity: '⚡', Education: '🎓', Rescue: '🚒',
  Transport: '🚌', Municipal: '🏛️', Environment: '🌿'
}
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm border border-white/30 text-white text-xs font-medium px-3 py-1.5 rounded-full mb-4">
            <span>Processed by local keyword parser</span>
            <span className="w-1 h-1 bg-white/60 rounded-full"></span>
            <span>Results in under 3 seconds</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3 leading-tight drop-shadow-sm">
            Your complaint, <span className="text-yellow-200">heard instantly</span>
          </h1>
              ...
              {['Write complaint','AI processes','Report generated'].map((label, i) => (
                <div key={i} className="flex items-center">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-xs text-white/70 hidden sm:block drop-shadow-sm">{label}</span>
                  </div>
                  {i < 2 && <div className={`w-12 sm:w-20 h-0.5 mx-2 mb-4 transition-all ${result ? 'bg-white/60' : 'bg-white/30'}`}/>}
                </div>
              ))}
            </div>
        ...
        {/* Footer */}
        <p className="text-center text-xs text-white/80 mt-8 pb-4 drop-shadow-sm">
